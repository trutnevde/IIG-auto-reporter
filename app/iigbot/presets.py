...
SETTINGS = [
    ...
    {"id": "ENABLE_COMPANY_INFO",
     ...
     "src": "api", "default": "YES"},
    # Документация Директа называет этот параметр отвечающим за приоритизацию объявлений:
    # включено — показывается объявление с фразой, наиболее близкой к запросу, ставка не
    # учитывается; выключено — выбор идёт по ставке.
    {"id": "CAMPAIGN_EXACT_PHRASE_MATCHING_ENABLED",
     "name": "Приоритизация объявлений: по фразе, наиболее близкой к запросу",
     "src": "ui", "default": "NO"},
...
NOT_IN_API = [
    {"name": "Блок «Директ помогает» целиком",
     "why": "через API доступна только одна его настройка — ALTERNATIVE_TEXTS_ENABLED "
            "(«Оптимизировать текст объявлений»); остальное правится в интерфейсе"},
]
