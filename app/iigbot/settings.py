...
# LSCCD = «последний значимый переход, кросс-девайс» — рекомендация Мари (29.07.2026).
# Кросс-девайс в API пишется через ДВА C: LSCCD/FCCD/LYDCCD. Вариантов LSCD/LCD/FCD/LYDCD/LCCD
# не существует — Reports API отвечает 400 «неверное значение перечисления» (проверено вживую).
#
# Важно: Директ подменяет часть моделей на кросс-девайсные сам. Запрашиваешь LSC — в ответе
# колонка Conversions_<id>_LSCCD, FC -> FCCD, LYDC -> AUTO. То есть отчёты и так считались
# «последним значимым с кросс-девайсом»; смена LSC -> LSCCD цифры не двигает, она лишь делает
# намерение явным. А вот таблицы, стоявшие на LYDC, фактически считались по AUTO — вот из-за
# чего их цифры не сходились с отчётами.
DEFAULT_ATTRIBUTION = "LSCCD"


def default_attribution():
    """Модель атрибуции по умолчанию — из Настроек, иначе LSCCD."""
    try:
